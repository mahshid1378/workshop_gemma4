# Step 5 — Cache the model weights into GCS · L4 · E4B track
...
> Gemma 4 E4B weights from HuggingFace and uploads them into the GCS bucket from
...
| | |
|---|---|
| ⏱ **Time** | **~5–10 min** for E4B (it's ~16 GB) |
| 💰 **Cost** | A few cents of Cloud Build time + ~$0.32/mo storage until cleanup |

**Prerequisites**
- Steps 1–4 done; `HF_TOKEN` is real and you **accepted the E4B terms** (step 6 of prerequisites)
...
options:
  # L4 · E4B track: E4B is small, so a modest build machine and disk are fine.
  # (The RTX 31B track uses E2_HIGHCPU_32 / 500GB — same shape, bigger numbers.)
...
> ℹ️ **403 Forbidden during download?** You haven't accepted the Gemma 4 E4B
> license on HuggingFace with the account that owns this token. Fix it at
> <https://huggingface.co/google/gemma-4-E4B-it>, then re-run the submit.
...
# Sanity-check the total size — expect ~16 GB for E4B.
