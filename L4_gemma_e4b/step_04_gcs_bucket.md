# Step 4 — Create the regional GCS bucket · L4 · E4B track
...
> them at cold start instead of pulling ~16 GB from HuggingFace every time.
>
...
|---|---|
| ⏱ **Time** | ~1 minute |
| 💰 **Cost** | ~$0 now; storage is ~$0.02/GB/mo. 16 GB ≈ ~$0.32/mo if you forget to delete it. `step_99` cleans it up. |

**Prerequisites**
- Step 1 done (`MODEL_CACHE_BUCKET`, `GOOGLE_CLOUD_REGION` set)
- Step 2 done (`storage.googleapis.com` enabled)

> ℹ️ Sharing a project with the 12B L4 track? The bucket name is the same, so
