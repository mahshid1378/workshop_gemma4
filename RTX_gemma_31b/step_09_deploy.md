# Step 9 — Deploy to Cloud Run · RTX track
...
> Run service: it pulls the vLLM image, attaches an RTX PRO 6000, loads the 31B
> weights from GCS over Direct VPC Egress, and exposes a standard `/v1/chat/completions` HTTP API.
>
...
> few minutes while the weights stream in — that's why step 8 set a generous
> startup probe delay.

| | |
|---|---|
| ⏱ **Time** | ~5–8 min (most of it is the first cold-start model load for 31B) |
| 💰 **Cost** | **Billing starts now** — ~$3.19/hr while an instance is running. Scales to zero when idle, but **run `step_99` after the workshop.** |
...
- `--gpu=1` + `--gpu-type` — attach one RTX PRO 6000. **This is the headline
  difference between tracks**; L4 tracks pass `nvidia-l4` here instead.
