# Step 5 — Cache the model weights into GCS · L4 · 12B track
...
> Gemma 4 12B weights from HuggingFace and uploads them into the GCS bucket from
...
> makes cold starts in step 9 fast. Running it in Cloud Build (not in Cloud
> Shell) means the transfer happens on a big build VM, not your laptop's session.

| | |
|---|---|
| ⏱ **Time** | **~8–12 min** for 12B (it's ~28 GB — bigger than E4B) |
| 💰 **Cost** | A few cents of Cloud Build time + ~$0.56/mo storage until cleanup |

**Prerequisites**
- Steps 1–4 done; `HF_TOKEN` is real and you **accepted the 12B terms** (step 6 of prerequisites)
...
options:
  # L4 · 12B track: 12B is ~28 GB on disk, so we bump the build disk to 150GB.
  # The build machine stays modest. (The RTX 31B track uses E2_HIGHCPU_32 / 500GB
  # — same shape, bigger numbers; the E4B L4 track uses 100GB.)
...
> ℹ️ **403 Forbidden during download?** You haven't accepted the Gemma 4 12B
> license on HuggingFace with the account that owns this token. Fix it at
> <https://huggingface.co/google/gemma-4-12B-it>, then re-run the submit.
...
# Sanity-check the total size — expect ~28 GB for 12B.
